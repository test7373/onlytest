...
//first ends 
// ==UserScript==
// @id             iitc-plugin-player-tracker-with-names@breunigs
// @name           IITC Plugin: Player tracker WITH NAMES
// @category       Layer
// @version        0.11.1.20150917.154202
// @namespace      https://github.com/jonatkins/ingress-intel-total-conversion
// @description    [jonatkins-2015-09-17-154202] with names Draw trails for the path a user took onto the map based on status messages in COMMs. Uses up to three hours of data. Does not request chat data on its own, even if that would be useful.
...
var thisplugin = window.plugin.playerTrackerWithNames = function() {};

    thisplugin.enlplayers=0;
    thisplugin.resplayers=0;
window.plugin.playerTrackerWithNames.setup = function() {

    
    jQuery("<style>")
      .prop("type", "text/css")
      .html(".leaflet-marker-icon-wrapper {\
	position: absolute\
}\
\
...
.enlight {\
    ...
    jQuery("<style>")
      .prop("type", "text/css")
      .html(".plugin-importgps-marker-text {\
              font-size: 12px;\
              //color: #FFFFBB;\
              color: #AA0011;\
              font-family: 'roboto',arial,helvetica,sans-serif;\
              text-align: center;\
              //text-shadow: 0px -1px 0px white, 1px 0px 0px white, 0px 1px 0px white, -1px 0px 0px white;\
  text-shadow: \
    -0   -2px 1px #FFFFFF,\
     0   -2px 1px #FFFFFF,\
    -0    2px 1px #FFFFFF,\
    ...
    -2px  2px 1px #FFFFFF,\
     ...
     jQuery("<style>")
      .prop("type", "text/css")
      .html(".plugin-importgps-marker-text1 {\
              font-size: 20px;\
              //color: #FFFFBB;\
              color: #AA0011;\
              font-family: 'roboto',arial,helvetica,sans-serif;\
  ...
  plugin.playerTrackerWithNames.iconEnl = L.Icon.Default.extend({options: {
    iconUrl: iconEnlImage,
    iconRetinaUrl: iconEnlRetImage
  }});
  plugin.playerTrackerWithNames.iconRes = L.Icon.Default.extend({options: {
    iconUrl: iconResImage,
    iconRetinaUrl: iconResRetImage
  }});

  plugin.playerTrackerWithNames.drawnTracesEnl = new L.LayerGroup();
  plugin.playerTrackerWithNames.drawnTracesRes = new L.LayerGroup();
  // to avoid any favouritism, we'll put the player's own faction layer first
  if (PLAYER.team == 'RESISTANCE') {
    window.addLayerGroup('Player Tracker Res +names', plugin.playerTrackerWithNames.drawnTracesRes, true);
    window.addLayerGroup('Player Tracker Enl +names', plugin.playerTrackerWithNames.drawnTracesEnl, true);
  } else {
    window.addLayerGroup('Player Tracker Enl +names', plugin.playerTrackerWithNames.drawnTracesEnl, true);
    window.addLayerGroup('Player Tracker Res +names', plugin.playerTrackerWithNames.drawnTracesRes, true);
  }
  map.on('layeradd',function(obj) {
    if(obj.layer === plugin.playerTrackerWithNames.drawnTracesEnl || obj.layer === plugin.playerTrackerWithNames.drawnTracesRes) {
  ...
  plugin.playerTrackerWithNames.playerPopup = new L.Popup({offset: L.point([1,-34])});

  addHook('publicChatDataAvailable', window.plugin.playerTrackerWithNames.handleData);

  window.map.on('zoomend', function() {
    window.plugin.playerTrackerWithNames.zoomListener();
  });
  window.plugin.playerTrackerWithNames.zoomListener();
  
  plugin.playerTrackerWithNames.setupUserSearch();
}

window.plugin.playerTrackerWithNames.stored = {};

plugin.playerTrackerWithNames.onClickListener = function(event) {
  var marker = event.target;

  if (marker.options.desc) {
    plugin.playerTrackerWithNames.playerPopup.setContent(marker.options.desc);
    plugin.playerTrackerWithNames.playerPopup.setLatLng(marker.getLatLng());
    map.openPopup(plugin.playerTrackerWithNames.playerPopup);
  }
};

// force close all open tooltips before markers are cleared
window.plugin.playerTrackerWithNames.closeIconTooltips = function() {
  plugin.playerTrackerWithNames.drawnTracesRes.eachLayer(function(layer) {
    if ($(layer._icon)) { $(layer._icon).tooltip('close');}
  });
  plugin.playerTrackerWithNames.drawnTracesEnl.eachLayer(function(layer) {
    if ($(layer._icon)) { $(layer._icon).tooltip('close');}
  });
}

window.plugin.playerTrackerWithNames.zoomListener = function() {
  ...
  if(window.map.getZoom() < window.PLAYER_TRACKER_MIN_ZOOM) {
    if (!window.isTouchDevice()) plugin.playerTrackerWithNames.closeIconTooltips();
    plugin.playerTrackerWithNames.drawnTracesEnl.clearLayers();
    plugin.playerTrackerWithNames.drawnTracesRes.clearLayers();
    ctrl.addClass('disabled').attr('title', 'Zoom in to show those.');
    //note: zoomListener is also called at init time to set up things, so we only need to do this in here
    window.chat.backgroundChannelData('plugin.playerTrackerWithNames', 'all', false);   //disable this plugin's interest in 'all' COMM
  } else {
    ctrl.removeClass('disabled').attr('title', '');
    //note: zoomListener is also called at init time to set up things, so we only need to do this in here
    window.chat.backgroundChannelData('plugin.playerTrackerWithNames', 'all', true);    //enable this plugin's interest in 'all' COMM
  }
}

window.plugin.playerTrackerWithNames.getLimit = function() {
 return new Date().getTime() - window.PLAYER_TRACKER_MAX_TIME;
}

window.plugin.playerTrackerWithNames.discardOldData = function() {
  var limit = plugin.playerTrackerWithNames.getLimit();
  $.each(plugin.playerTrackerWithNames.stored, function(plrname, player) {
    var i;
    ...
    if(i === 0) return true;
    if(i === ev.length) return delete plugin.playerTrackerWithNames.stored[plrname];
    plugin.playerTrackerWithNames.stored[plrname].events.splice(0, i);
  });
}

window.plugin.playerTrackerWithNames.eventHasLatLng = function(ev, lat, lng) {
  var hasLatLng = false;
...
window.plugin.playerTrackerWithNames.processNewData = function(data) {
  var limit = plugin.playerTrackerWithNames.getLimit();
  $.each(data.result, function(ind, json) {
    ...
    var playerData = window.plugin.playerTrackerWithNames.stored[plrname];

    // short-path if this is a new player
    if(!playerData || playerData.events.length === 0) {
      plugin.playerTrackerWithNames.stored[plrname] = {
        nick: plrname,
    ...
    if(evts[cmp].time === json[1]) {
      ...
      plugin.playerTrackerWithNames.stored[plrname].events = evts;
      return true;
    }
    ...
    if(evts[cmp+1] && plugin.playerTrackerWithNames.eventHasLatLng(evts[cmp+1], lat, lng))
      return true;

    // if this event is newer, need to look at the previous one
    var sameLocation = plugin.playerTrackerWithNames.eventHasLatLng(evts[cmp], lat, lng);
...
window.plugin.playerTrackerWithNames.getLatLngFromEvent = function(ev) {
...
window.plugin.playerTrackerWithNames.ago = function(time, now) {
  var s = (now-time) / 1000;
...
window.plugin.playerTrackerWithNames.drawData = function() {
    this.enlplayers=0;
    this.resplayers=0;
    
  var isTouchDev = window.isTouchDevice();

  var gllfe = plugin.playerTrackerWithNames.getLatLngFromEvent;
  ...
  $.each(plugin.playerTrackerWithNames.stored, function(plrname, playerData) {
      if(playerData.team==="RESISTANCE")
      {
            thisplugin.resplayers ++;
      }
      else
          thisplugin.enlplayers ++;
    if(!playerData || playerData.events.length === 0) {
    ...
    var ago = plugin.playerTrackerWithNames.ago;
    ...
    popup
      ...
      .append('<br>')
      .append(plugin.playerTrackerWithNames.getPortalLink(last));
    ...
    if(evtsLength >= 2) {
      ...
      for(var i = evtsLength - 2; i >= 0 && i >= evtsLength - 10; i--) {
        ...
        $('<tr>')
          ...
          .append($('<td>')
            .append(plugin.playerTrackerWithNames.getPortalLink(ev)))
          .appendTo(table);
      }
    }
  ...
  var iconResRetImage = 'data:image/png;base64,';
    ...
    var iconUrl = playerData.team === 'RESISTANCE' ?  iconResImage :  iconEnlImage;
    var color = playerData.team === 'RESISTANCE' ?  'resist' :  'enlight';
    ...
    // marker click events. so store the popup text in the options, then display it in the oms click handler
      ...
      var icon=new L.Icon.Label.Default({
          iconUrl: iconUrl,
          labelText:plrname+", "+ago(last.time, now),
          iconAnchor: new L.Point(0, 0),
          labelAnchor: new L.Point(-12, -20),
          wrapperAnchor: new L.Point(12 ,41),
          shadowUrl:null,
          iconSize: new L.Point(25, 41),
    ...
    var m = L.marker(gllfe(last), {icon: icon, referenceToPortal: closestPortal, opacity: absOpacity, desc: popup[0], title: tooltip,zIndexOffset: 1000});           
      
     m.addEventListener('spiderfiedclick', plugin.playerTrackerWithNames.onClickListener);
      if (tooltip) {
      // ensure tooltips are closed, sometimes they linger
      m.on('mouseout', function() { $(this._icon).tooltip('close'); });
    }
      m.addTo(playerData.team === 'RESISTANCE' ? plugin.playerTrackerWithNames.drawnTracesRes : plugin.playerTrackerWithNames.drawnTracesEnl);
    ...
    /*var m = L.marker(gllfe(last), {
        icon: L.divIcon({
          className: 'plugin-importgps-marker-text',
          //iconAnchor: [40, 10],
          iconAnchor: [40, 0],
          iconSize: [80, 40],
          html: plrname
    ...
    //var m = L.marker(gllfe(last), {opacity: absOpacity, desc: popup[0], title: tooltip,html:"FUCK!"});
    m.addEventListener('spiderfiedclick', plugin.playerTrackerWithNames.onClickListener);
...
*/
  ...
  $.each(polyLineByAgeEnl, function(i, polyLine) {
    ...
    $.each(polyLine,function(ind,poly) {
      L.polyline(poly, opts).addTo(plugin.playerTrackerWithNames.drawnTracesEnl);
    });
  });
  $.each(polyLineByAgeRes, function(i, polyLine) {
    ...
    $.each(polyLine, function(ind,poly) {
      L.polyline(poly, opts).addTo(plugin.playerTrackerWithNames.drawnTracesRes);
    });
  });
    
    console.log("ENL PLAYERS: ",thisplugin.enlplayers);
    console.log("RES PLAYERS: ",thisplugin.resplayers);
}

window.plugin.playerTrackerWithNames.getPortalLink = function(data) {
...
window.plugin.playerTrackerWithNames.handleData = function(data) {
  if(window.map.getZoom() < window.PLAYER_TRACKER_MIN_ZOOM) return;

  plugin.playerTrackerWithNames.discardOldData();
  plugin.playerTrackerWithNames.processNewData(data);
  if (!window.isTouchDevice()) plugin.playerTrackerWithNames.closeIconTooltips();

  plugin.playerTrackerWithNames.drawnTracesEnl.clearLayers();
  plugin.playerTrackerWithNames.drawnTracesRes.clearLayers();
  plugin.playerTrackerWithNames.drawData();
}

window.plugin.playerTrackerWithNames.findUser = function(nick) {
...
window.plugin.playerTrackerWithNames.findUserPosition = function(nick) {
  var data = window.plugin.playerTrackerWithNames.findUser(nick);
  if (!data) return false;

  var last = data.events[data.events.length - 1];
  return plugin.playerTrackerWithNames.getLatLngFromEvent(last);
}

window.plugin.playerTrackerWithNames.centerMapOnUser = function(nick) {
  var data = plugin.playerTrackerWithNames.findUser(nick);
  if(!data) return false;

  var last = data.events[data.events.length - 1];
  var position = plugin.playerTrackerWithNames.getLatLngFromEvent(last);

  if(window.isSmartphone()) window.show('map');
  window.map.setView(position, map.getZoom());

  if(data.marker) {
    window.plugin.playerTrackerWithNames.onClickListener({target: data.marker});
  }
  return true;
}

window.plugin.playerTrackerWithNames.onNicknameClicked = function(info) {
  if (info.event.ctrlKey || info.event.metaKey) {
    return !plugin.playerTrackerWithNames.centerMapOnUser(info.nickname);
  }
  return true; // don't interrupt hook
}

window.plugin.playerTrackerWithNames.onSearchResultSelected = function(result, event) {
  ...
  if(false === window.plugin.playerTrackerWithNames.centerMapOnUser(result.nickname))
...
window.plugin.playerTrackerWithNames.onSearch = function(query) {
  var term = query.term.toLowerCase();

  if (term.length && term[0] == '@') term = term.substr(1);

  $.each(plugin.playerTrackerWithNames.stored, function(nick, data) {
    ...
    query.addResult({
      ...
      position: plugin.playerTrackerWithNames.getLatLngFromEvent(event),
      onSelected: window.plugin.playerTrackerWithNames.onSearchResultSelected,
    });
  });
}

window.plugin.playerTrackerWithNames.setupUserSearch = function() {
  addHook('nicknameClicked', window.plugin.playerTrackerWithNames.onNicknameClicked);
  addHook('search', window.plugin.playerTrackerWithNames.onSearch);
}


var setup = plugin.playerTrackerWithNames.setup;
